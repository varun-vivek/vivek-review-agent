...
public class TsChunkHelper {

    private static final int MAX_CHUNK_SIZE = 100; // non-empty lines

    // Regex patterns for TypeScript/JavaScript function/method detection
    private static final List<Pattern> FUNCTION_PATTERNS = Arrays.asList(
            // function declaration: function name() {}
            Pattern.compile("^\\s*(?:export\\s+)?(?:async\\s+)?function\\s+\\w+\\s*\\([^)]*\\)\\s*(?::\\s*[\\w<>\\[\\]|&\\s]+)?\\s*\\{?\\s*$"),
            // arrow function: const name = () => {}
            Pattern.compile("^\\s*(?:export\\s+)?(?:const|let|var)\\s+\\w+\\s*[:=]\\s*(?:\\([^)]*\\)\\s*=>|\\w+\\s*=>)\\s*\\{?\\s*$"),
            // method in class: methodName() {}
            Pattern.compile("^\\s*(?:public|private|protected)?\\s*(?:static)?\\s*(?:async)?\\s*\\w+\\s*\\([^)]*\\)\\s*(?::\\s*[\\w<>\\[\\]|&\\s]+)?\\s*\\{?\\s*$"),
            // class declaration: class Name {}
            Pattern.compile("^\\s*(?:export\\s+)?(?:abstract\\s+)?class\\s+\\w+"),
            // interface declaration: interface Name {}
            Pattern.compile("^\\s*(?:export\\s+)?interface\\s+\\w+"),
            // type declaration: type Name =
            Pattern.compile("^\\s*(?:export\\s+)?type\\s+\\w+\\s*="),
            // enum declaration: enum Name {}
            Pattern.compile("^\\s*(?:export\\s+)?enum\\s+\\w+")
    );
    ...
    public List<String> processDiffLines(String diffString) {
        ...
        // Group into functions/blocks
        List<CodeBlock> codeBlocks = identifyCodeBlocks(diffLines);

        // Create chunks respecting function boundaries and size limits
    ...
    private List<CodeBlock> identifyCodeBlocks(List<DiffLine> diffLines) {
        ...
        while (i < diffLines.size()) {
            ...
            if (isFunctionStart(currentLine.content)) {
                // Find the complete function
                CodeBlock functionBlock = findCompleteFunction(diffLines, i);
                blocks.add(functionBlock);
                i = functionBlock.endIndex + 1;
            } else {
                // Single line or group of non-function lines
    ...
    private boolean isFunctionStart(String line) {
        ...
        // Skip comments, imports, type-only exports
        if (trimmed.startsWith("//") || trimmed.startsWith("/*") ||
                trimmed.startsWith("import ") || trimmed.startsWith("export type") ||
                trimmed.startsWith("export interface") || trimmed.isEmpty()) {
            return false;
        }

        // Check against all TypeScript function patterns
        return FUNCTION_PATTERNS.stream().anyMatch(pattern -> pattern.matcher(line).find());
    }

    private CodeBlock findCompleteFunction(List<DiffLine> diffLines, int startIndex) {
        int braceCount = 0;
        ...
        for (int i = startIndex; i < diffLines.size(); i++) {
            ...
            // If we found the opening brace and braces are balanced, function is complete
            if (foundOpenBrace && braceCount == 0) {
                break;
            }

            // For arrow functions or single-line functions without braces
            if (!foundOpenBrace && i > startIndex &&
                    (line.trim().endsWith(";") || isFunctionStart(line))) {
                endIndex = i - 1;
                break;
            }
        }

        return new CodeBlock(startIndex, endIndex, "FUNCTION");
    }
    ...
    private void outputChunks(List<String> chunks) {
        System.out.println("Generated " + chunks.size() + " TypeScript chunks:");
